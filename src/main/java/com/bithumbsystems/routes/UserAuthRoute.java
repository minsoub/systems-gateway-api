        ...
        return builder.routes()
                ...
                .route("auth-service",
                        route ->route.path("/auth/**")
                                .filters(filter -> filter.filter(new AuthFilter().apply(new Config("AuthFilter apply", true, true))))
                                .uri(authUrl)
                )
                .route("api-service",   // API 서비스 호출
                        route -> route.path("/api/**")
                                .filters(filter -> filter.filter(new ApiFilter().apply(new Config("ApiFilter apply", true, true))))
                                .uri(smartAdminUrl)
                ).build();
    }
}
