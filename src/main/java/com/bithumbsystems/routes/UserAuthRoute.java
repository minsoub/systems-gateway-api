...
public class UserAuthRoute {
    ...
    public RouteLocator routes(RouteLocatorBuilder builder) {
        ...
        return builder.routes()
                ...
                .route("api-service-mng-cpc",   // API 서비스 호출 (CPC Smart Admin)
                        route -> route.path("/api/*/mng/cpc/**")
                                .filters(filter -> filter.filter(apiFilter.apply(new Config("LRC Smart Admin ApiFilter apply", true, true))))
                                .uri(urlProperties.getSmartAdminCpcUrl())
                )
                .route("api-service-mng-cms",   // API 서비스 호출 (CMS Smart Admin)
                        route -> route.path("/api/*/mng/cms/**")
                                .filters(filter -> filter.filter(apiFilter.apply(new Config("CMS Smart Admin ApiFilter apply", true, true))))
                                .uri(urlProperties.getSmartAdminCmsUrl())
                )
